...
def complementary_filter(angles, accel_raw, gyro_raw, gyro_bias, time_diff):
    '''
    All keys are expected to be x, y, z
    '''

    gyro_angle_x = angles['x'] + (gyro_raw['x']-gyro_bias['x'])*time_diff
    gyro_angle_y = angles['y'] + (gyro_raw['y']-gyro_bias['y'])*time_diff

    accelAngleX = math.atan2(accel_raw['y'], accel_raw['z']) * 180/3.141592
    accelAngleY = math.atan2(-accel_raw['x'], math.sqrt(accel_raw['y']*accel_raw['y'] + accel_raw['z']*accel_raw['z'])) * 180/3.141592

    # angles['x'] = 0.98*gyro_angle_x + 0.02*accelAngleX
    angles['x'] = 0.98*gyro_angle_x + 0.02*accelAngleX
    angles['y'] = 0.98*gyro_angle_y + 0.02*accelAngleY
...
if __name__ == "__main__":
    ...
    while time.time() -t9 < 30:
        ...
        t2 = time.time()
        complementary_filter(angles, accel_data, gyro_data, errors, t2-t3)
        t3 = time.time()
